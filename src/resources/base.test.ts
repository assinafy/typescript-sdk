...
describe('BaseResource helpers', () => {
    ...
    test('callList yields [] when the payload is neither array nor {data}', async () => {
        const r = await res().list$(async () => ({ status: 200, data: { status: 200, data: { not: 'a list' } }, headers: {} }));
        expect(r.data).toEqual([] as never);
    });
