...
- The manual live suite requires `--confirm-production` for `--all` on every configured host.
- Public examples use production URLs while retaining sandbox setup guidance.

## [2.4.1] - 2026-09-23

### Added

- OAuth's `webhooks:write` scope is available in `OAuthScope` and documented for
  webhook subscription updates and inactivation.
